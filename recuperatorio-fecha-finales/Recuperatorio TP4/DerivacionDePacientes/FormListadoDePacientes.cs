...
namespace DerivacionDePacientes
{
    public partial class FormListadoDePacientes : Form
    {
        ...
        public FormListadoDePacientes(Pacientes<Persona> pacientes)
        {
            ...
            this.dataGridPacientes.DataSource = this.dtPacientes;
        }
        ...
        private void btnDetallarConsulta_Click(object sender, EventArgs e)
        {
            try
            {
                FormDetallarConsulta detalleConsulta = new FormDetallarConsulta(this.pacientes.Listado);
                detalleConsulta.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
